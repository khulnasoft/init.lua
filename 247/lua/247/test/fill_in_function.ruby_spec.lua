---@module "plenary.busted"

-- luacheck: globals describe it assert
local _247 = require("247")
local test_utils = require("247.test.test_utils")
local Levels = require("247.logger.level")
local eq = assert.are.same

--- @param content string[]
--- @param row number
--- @param col number
--- @param lang string?
--- @return _247.test.Provider, number
local function setup(content, row, col, lang)
  assert(lang, "lang must be provided")
  local provider = test_utils.TestProvider.new()
  _247.setup({
    provider = provider,
    logger = {
      error_cache_level = Levels.ERROR,
      type = "print",
    },
  })

  local buffer = test_utils.create_file(content, lang, row, col)
  return provider, buffer
end

--- @param buffer number
--- @return string[]
local function read(buffer)
  return vim.api.nvim_buf_get_lines(buffer, 0, -1, false)
end

describe("fill_in_function", function()
  it("fill in ruby method", function()
    local ruby_content = {
      "",
      "def test() end",
    }
    local provider, buffer = setup(ruby_content, 2, 5, "ruby")
    local state = _247.__get_state()

    _247.fill_in_function()

    eq(1, state:active_request_count())
    eq(ruby_content, read(buffer))

    provider:resolve("success", "def test()\n    return 42\nend")
    test_utils.next_frame()

    local expected_state = {
      "",
      "def test()",
      "    return 42",
      "end",
    }
    eq(expected_state, read(buffer))
    eq(0, state:active_request_count())
  end)

  it("fill in ruby method with comment", function()
    local ruby_content = {
      "",
      "def calculate(x, y)",
      "    # TODO: return the sum",
      "end",
    }
    local provider, buffer = setup(ruby_content, 2, 5, "ruby")
    local state = _247.__get_state()

    _247.fill_in_function()

    eq(1, state:active_request_count())
    eq(ruby_content, read(buffer))

    provider:resolve("success", "def calculate(x, y)\n    x + y\nend")
    test_utils.next_frame()

    local expected_state = {
      "",
      "def calculate(x, y)",
      "    x + y",
      "end",
    }
    eq(expected_state, read(buffer))
    eq(0, state:active_request_count())
  end)
end)
