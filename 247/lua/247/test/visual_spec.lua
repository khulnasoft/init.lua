...
local test_utils = require("247.test.test_utils")
local eq = assert.are.same
local Levels = require("247.logger.level")
local Range = require("247.geo").Range
local Point = require("247.geo").Point
...
--- @return _247.test.Provider, number, _247.Range
...
describe("visual", function()
  it("should replace visual selection with AI response", function()
    ...
    local state = _247.__get_state()
    local visual_fn = require("247.ops.over-range")

    local context =
      require("247.request-context").from_current_buffer(state, 100)
  ...
  it("should handle multi-line replacement", function()
    ...
    local context =
      require("247.request-context").from_current_buffer(state, 200)
    visual_fn(context, range)
  ...
  it("should cancel request when stop_all_requests is called", function()
    ...
    local state = _247.__get_state()
    local context =
      require("247.request-context").from_current_buffer(state, 300)
  ...
  it("should handle error cases with graceful failures", function()
    ...
    local state = _247.__get_state()
    local context =
      require("247.request-context").from_current_buffer(state, 400)
  ...
  it("should handle cancelled status gracefully", function()
    ...
    local state = _247.__get_state()
    local context =
      require("247.request-context").from_current_buffer(state, 500)
