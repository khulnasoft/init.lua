...
local utils = require("247.utils")
local random_file = utils.random_file

--- @class _247.RequestContext
...
--- @field marks table<string, _247.Mark>
--- @field logger _247.Logger
--- @field xid number
--- @field range _247.Range?
...
function RequestContext.from_current_buffer(_247, xid)
  ...
  local mds = {}
  for _, md in ipairs(_247.md_files) do
    table.insert(mds, md)
  end
...
--- @param rules (_247.Agents.Rule | string)[]
...
function RequestContext:finalize()
  self:_read_md_files()
  if self.range then
    table.insert(self.ai_context, self._247.prompts.get_file_location(self))
    table.insert(self.ai_context, self._247.prompts.get_range_text(self.range))
  end
  table.insert(
    self.ai_context,
    self._247.prompts.tmp_file_location(self.tmp_file)
  )
  return self
end
