...
local Request = require("247.request")
local Mark = require("247.ops.marks")
local editor = require("247.editor")
local RequestStatus = require("247.ops.request_status")
local Window = require("247.window")
local make_clean_up = require("247.ops.clean-up")
...
local function fill_in_function(context, opts)
  ...
  local virt_line_count = context._247.ai_stdout_rows
  if virt_line_count >= 0 then
    context.marks.function_location = Mark.mark_func_body(buffer, func)
  end

  local request = Request.new(context)
  local full_prompt = context._247.prompts.prompts.fill_in_function()
  local additional_prompt = opts.additional_prompt
  if additional_prompt then
    full_prompt =
      context._247.prompts.prompts.prompt(additional_prompt, full_prompt)

    local rules = Agents.find_rules(context._247.rules, additional_prompt)
  ...
  request:start({
    ...
    on_complete = function(status, response)
      ...
      if status == "failed" then
        if context._247.display_errors then
